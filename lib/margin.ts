...
/**
 * Berekent voor één set orders (al gefilterd op periode) de contributiemarge
 * per order, op basis van gedeelde kostprijs/transactiekosten/verzendkosten-
 * lookup-tabellen die over de VOLLEDIGE opgehaalde periode (huidig +
 * vergelijking) zijn opgebouwd.
 */
...
export async function computeDashboardData(periodKey: PeriodKey): Promise<DashboardData> {
  ...
  // Eén keer ophalen vanaf het begin van de VERGELIJKINGSperiode — dat dekt
  // zowel de huidige als de vorige periode in één set API-calls.
  ...
  // Dagelijkse trend (huidige periode, oplopend gesorteerd)
